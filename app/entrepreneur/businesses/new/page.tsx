...
import { entrepreneurApi, apiClient } from "@/lib/api"
...
export default function NewBusinessPage() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const data = await apiClient<Category[]>("/entrepreneur/categories")
        setCategories(data)
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.title || !formData.category || !businessPlan) {
    ...
    try {
      ...
      data.append("businessPlan", businessPlan)
  ...
  return (
    <DashboardLayout>
      <div className="max-w-2xl mx-auto">
        <Card>
          <CardHeader>
            <CardTitle>Submit Business for Review</CardTitle>
            <CardDescription>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                  ...
                  <SelectContent>
                    {categories.map((cat) => (
                      <SelectItem key={cat._id} value={cat._id}>
                        {cat.name} (Fee: ${cat.registration_fee})
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                {selectedCategory && (
                  <p className="text-sm text-muted-foreground">
                    Registration fee for this category: <span className="font-semibold">${selectedCategory.registration_fee}</span>
                  </p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Upload your detailed business plan. This is essential for our review process.
                </p>
