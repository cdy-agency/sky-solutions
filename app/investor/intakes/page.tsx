...
export default function InvestorIntakesPage() {
  const { token } = useAuth()
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Manage your investor intake submissions</p>
          </div>
          <Link href="/investor/intake">
            <Button>
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="space-y-4">
                {intakes.map((intake) => (
                  <Link key={intake._id} href={`/investor/intakes/${intake._id}`}>
                    <div className="flex items-center justify-between p-4 rounded-lg border border-border hover:bg-muted/50 transition-colors">
                      <div className="flex items-center gap-3">
                        {getStatusIcon(intake.status)}
                        <div>
                          <h3 className="font-medium text-foreground">Investor Intake Form</h3>
                          <p className="text-sm text-muted-foreground">{intake.full_name}</p>
                        </div>
                      </div>
